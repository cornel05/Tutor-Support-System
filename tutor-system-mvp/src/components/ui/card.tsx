...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-lg backdrop-blur transition-colors dark:border-slate-800 dark:bg-slate-900/80',
        className
      )}
      {...props}
    />
...
const CardContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('space-y-4 text-slate-600 dark:text-slate-200', className)}
      {...props}
    />
  )
);
