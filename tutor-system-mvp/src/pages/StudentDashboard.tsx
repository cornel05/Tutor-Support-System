...
function StudentDashboard() {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-3xl border border-brand/30 bg-white p-8 text-slate-700 shadow-glow transition-colors dark:border-brand/40 dark:bg-gradient-to-r dark:from-brand-dark/60 dark:via-brand/40 dark:to-brand-light/40 dark:text-white/80">
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <div className="space-y-3">
            <p className="inline-flex items-center gap-2 rounded-full bg-brand/10 px-3 py-1 text-xs uppercase tracking-[0.3em] text-brand-dark transition-colors dark:bg-white/10 dark:text-white">
              <Sparkles className="h-4 w-4" /> Student HQ
            </p>
            <h2 className="text-3xl font-semibold text-slate-900 transition-colors dark:text-white md:text-4xl">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white/80 px-6 py-5 text-sm text-slate-600 transition-colors dark:border-white/10 dark:bg-white/10 dark:text-white/90">
            <p className="font-semibold text-slate-900 dark:text-white">Next session</p>
            {upcoming.length ? (
              <p>
              ...
              <p className="text-slate-500 dark:text-white/80">No sessions booked yet. Let's start!</p>
            )}
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-1">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-xs text-slate-500 transition-colors dark:border-slate-800 dark:bg-slate-900/70 dark:text-slate-400">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {filteredTutors.map((tutor) => (
                <div
                  key={tutor.id}
                  className="rounded-2xl border border-slate-200 bg-white p-4 transition hover:border-brand hover:shadow-glow dark:border-slate-800 dark:bg-slate-900/60"
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-lg font-semibold text-slate-900 dark:text-white">{tutor.name}</p>
                      <p className="text-xs uppercase tracking-widest text-slate-500 dark:text-slate-500">{tutor.major}</p>
                    </div>
                    <Badge variant="success" className="text-[11px]">
                      <Star className="mr-1 h-3 w-3" /> {tutor.rating?.toFixed(1)}
                    </Badge>
                  </div>
                  <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">{tutor.expertise?.join(' • ')}</p>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {myRequests.map((request) => (
                <div
                  key={request.id}
                  className="rounded-xl border border-slate-200 bg-slate-50 p-4 transition-colors dark:border-slate-800 dark:bg-slate-900/60"
                >
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">
                    {tutors.find((tutor) => tutor.id === request.tutorId)?.name}
                  </p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">{request.message}</p>
                  <Badge
                    variant={
            ...
            <CardContent className="space-y-4">
              {resources.slice(0, 3).map((resource) => (
                <a
                  key={resource.id}
                  href={resource.url}
                  className="block rounded-xl border border-slate-200 bg-white p-4 text-slate-600 transition hover:border-brand hover:text-brand-dark dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-300 dark:hover:text-white"
                >
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-[10px] uppercase tracking-widest text-brand-dark dark:text-brand-light">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {upcoming.map((session) => (
              <div
                key={session.id}
                className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 transition-colors md:flex-row md:items-center md:justify-between dark:border-slate-800 dark:bg-slate-900/60"
              >
                <div>
          ...
          <CardContent className="space-y-4">
            {past.map((session) => {
              ...
              return (
                <div
                  key={session.id}
                  className="rounded-2xl border border-slate-200 bg-white p-4 transition-colors dark:border-slate-800 dark:bg-slate-900/60"
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="font-semibold text-slate-900 dark:text-white">{tutor?.name}</p>
                  ...
                  {existingFeedback && <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">“{existingFeedback.comment}”</p>}
                </div>
              );
            })}
