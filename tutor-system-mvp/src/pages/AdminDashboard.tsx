...
function AdminDashboard() {
  ...
  return (
    <div className="space-y-8">
      <section className="rounded-3xl border border-slate-200 bg-white p-8 transition-colors dark:border-slate-800 dark:bg-gradient-to-r dark:from-brand-dark/60 dark:via-brand/40 dark:to-slate-900">
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            <p className="text-sm uppercase tracking-[0.3em] text-brand-dark/80 dark:text-white/70">Admin command center</p>
      ...
      <div className="grid gap-6 xl:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-semibold text-slate-900 dark:text-white">{students.length}</p>
            <p className="text-xs text-slate-500 dark:text-slate-500">+3 since last week</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <p className="text-3xl font-semibold text-slate-900 dark:text-white">{tutors.length}</p>
            <p className="text-xs text-slate-500 dark:text-slate-500">92% availability synced</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            <div>
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Tutors</h3>
              <ul className="mt-2 space-y-2 text-sm text-slate-600 dark:text-slate-400">
                {tutors.slice(0, 5).map((tutor) => (
                  <li
                    key={tutor.id}
                    className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 transition-colors dark:border-slate-800 dark:bg-slate-900/60"
                  >
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Students</h3>
              <ul className="mt-2 space-y-2 text-sm text-slate-600 dark:text-slate-400">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {latestRequests.map((request) => (
              <div
                key={request.id}
                className="rounded-2xl border border-slate-200 bg-slate-50 p-4 transition-colors dark:border-slate-800 dark:bg-slate-900/60"
              >
                <p className="text-sm font-semibold text-slate-900 dark:text-white">
                  {students.find((student) => student.id === request.studentId)?.name} →{' '}
                  {tutors.find((tutor) => tutor.id === request.tutorId)?.name}
                </p>
                <p className="text-xs text-slate-500 dark:text-slate-400">{format(new Date(request.createdAt), 'MMM d, HH:mm')}</p>
                <Badge
                  variant={
            ...
            {!latestRequests.length && <p className="text-sm text-slate-500 dark:text-slate-500">No recent activity.</p>}
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="grid gap-4 md:grid-cols-3">
          {resources.map((resource) => (
            <a
              key={resource.id}
              href={resource.url}
              className="rounded-2xl border border-slate-200 bg-white p-4 text-slate-600 transition hover:border-brand hover:text-brand-dark dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-300 dark:hover:text-white"
            >
              ...
              <div className="mt-2 text-[10px] uppercase tracking-widest text-brand-dark dark:text-brand-light">
                {resource.tags.join(' · ')}
              </div>
            </a>
          ))}
